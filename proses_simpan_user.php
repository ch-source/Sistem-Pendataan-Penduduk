...
if ($result > 0) {
   echo "<script>alert('Proses Registrasi Gagal!, Username Yang Anda Masukkan Sudah Digunakan, Masukkan Username Yang Berbeda');
    document.location.href='dashboard.php?p=input_users'</script>\n";
      ...
      $query2="INSERT INTO `tbl_user` (`id_user`, `id_warga`, `nama_user`, `telepon_user`, `email_user`, `username`, `password`, `level`, `tgl_registrasi`, `status_user`) VALUES ('$iduser', '', '$nama', '$telepon', '$email', '$username', '$password', '$jabatan', '".date('Y-m-d')."', '')";
